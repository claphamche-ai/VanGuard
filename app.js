// ==========================================
// VANGUARD V1.1.26 - FINAL MASTER SCRIPT FIX
// ==========================================
const BRAND_NAME = "VanGuard";

const ASSET_ICONS = [
    {val: '🏢', label: 'Commercial Building'}, {val: '🛍️', label: 'Suburban Shopping Center'},
    {val: '📡', label: 'Telecommunications'}, {val: '🏘️', label: 'Community Center'},
    {val: '🎛️', label: 'CSL/Traffic Management Box'}, {val: '📬', label: 'Mail Box'},
    {val: '🅿️', label: 'Car Park'}, {val: '🎡', label: 'Playground'},
    {val: '🤺', label: 'Private Fence'}, {val: '🏠', label: 'Private Garage'},
    {val: '🏡', label: 'Private House'}, {val: '🪧', label: 'Private Signage'},
    {val: '🚧', label: 'Public Fence'}, {val: '📢', label: 'Public Signage'},
    {val: '🛣️', label: 'Alley Way'}, {val: '🎨', label: 'Public Mural'},
    {val: '🌉', label: 'Bridge'}, {val: '🚇', label: 'Underpass'},
    {val: '🎢', label: 'Overpass'}, {val: '🚌', label: 'Bus Shelter'},
    {val: '⚡', label: 'Power Pole'}, {val: '💡', label: 'Light Pole'},
    {val: '📞', label: 'Coms Pole'}, {val: '💈', label: 'Other Pole'},
    {val: '🚇', label: 'Tunnel'}, {val: '📍', label: 'Street Signage'},
    {val: '🚦', label: 'Traffic Signage'}, {val: '🛹', label: 'Skate Ramp'},
    {val: '🛋️', label: 'Park Bench'}, {val: '🗑️', label: 'Rubbish Bin'},
    {val: '🚗', label: 'Road'}, {val: '🚶', label: 'Footpath'},
    {val: '🧺', label: 'Picnic Table'}, {val: '📦', label: 'Skip Bin'},
    {val: '🚢', label: 'Shipping Container'}, {val: '🔋', label: 'Substation'},
...
const ReporterCtrl = {
    ...
    init: function() { 
        ...
        const fromInp = document.getElementById('rep-date-from'); const toInp = document.getElementById('rep-date-to');
        if(fromInp && !fromInp.value) { const d = new Date(); d.setDate(d.getDate() - 30); fromInp.valueAsDate = d; }
        if(toInp && !toInp.value) { toInp.valueAsDate = new Date(); }
    ...
    generatePreview: function() {
        ...
        const selectedWorkers = Array.from(document.querySelectorAll('.rep-worker-cb:checked')).map(cb => cb.value); const selectedMetrics = Array.from(document.querySelectorAll('.rep-metric-cb:checked')).map(cb => cb.value);
        ...
        let jobs = CoreDB.getTenantJobs().filter(j => {
            ...
            if(filterTag) { let content = (j.site + " " + (j.srn||"") + " " + (j.notes||"")).toLowerCase(); if(j.data) { for(let val of Object.values(j.data)) { content += " " + String(val).toLowerCase(); } } if(!content.includes(filterTag)) return false; }
            return true;
        });

        this.filteredJobsForPreview = jobs; const previewList = document.getElementById('report-preview-list'); if(jobs.length === 0) { previewList.innerHTML = `<p style="text-align:center; padding:30px; color:#888;">No jobs found. Try expanding your date range or selecting all workers.</p>`; } 
        else {
            const schema = CoreDB.getSchema(); const users = CoreDB.getUsers(); let html = '';
            jobs.forEach(j => {
                const user = users.find(u => u.id === j.assignedTo); const workerName = user ? (user.name || user.username) : (j.type === 'PENDING' ? 'Unassigned' : 'Unknown'); const isEdited = j.auditLog && j.auditLog.length > 0;
                ...
                if(j.photos && Object.keys(j.photos).length > 0) {
                    ...
                    for(const [step, url] of Object.entries(j.photos)) { html += `<div style="flex: 0 0 auto; text-align:center; cursor:pointer;" onclick="UI.openLightbox(${lbArrayStr}, ${pIndex})"><img src="${url}" style="height:120px; border-radius:5px; border:1px solid #ccc;"><div style="font-size:10px; color:#666; margin-top:5px; font-weight:bold;">${step.toUpperCase()}</div></div>`; pIndex++; }
                    html += `</div>`;
                }
                html += `<div style="padding:15px; display:grid; grid-template-columns:1fr 1fr; gap:10px;"><div style="font-size:12px;"><strong style="color:var(--b);">Date:</strong><br>${j.pausedAt || new Date(j.timestamp).toLocaleString('en-NZ')}</div>`;
                ...
                html += `<div style="padding:15px; background:#f4f6f8; border-top:1px solid #eee; display:flex; align-items:center; gap:20px;"><label style="font-size:13px; font-weight:bold;"><input type="radio" name="rev_${j.jobId}" value="APPROVED" ${isApp?'checked':''} onchange="ReporterCtrl.setJobReview('${j.jobId}', 'APPROVED')"> Approve</label><label style="font-size:13px; font-weight:bold; color:#e74c3c;"><input type="radio" name="rev_${j.jobId}" value="REJECTED" ${isRej?'checked':''} onchange="ReporterCtrl.setJobReview('${j.jobId}', 'REJECTED')"> Reject</label><textarea id="rej_reason_${j.jobId}" class="std-input" placeholder="Rejection reason..." style="display:${isRej?'block':'none'}; margin:0; height:30px; padding:5px; flex-grow:1;" onchange="ReporterCtrl.setRejectReason('${j.jobId}', this.value)">${j.rejectReason||''}</textarea></div></div>`;
            });
...
const OverlayMakerCtrl = {
    ...
    init: function() {
        ...
        // Populate the icon dropdown from the global ASSET_ICONS array
        const sel = document.getElementById('overlay-icon');
        if(sel) { sel.innerHTML = ASSET_ICONS.map(ic => `<option value="${ic.val}">${ic.val} ${ic.label}</option>`).join(''); }
    },
