...
const CoreDB = {
    ...
    pushReport: function(reportObj) {
        let r = this.getReports();
        reportObj.tenantId = this.getActiveTenantId();
        reportObj.id = 'REP-' + Date.now();
        reportObj.timestamp = new Date().toLocaleString('en-NZ');
        reportObj.status = 'NEW';
        r.unshift(reportObj);
        this.saveReports(r);
    }
};
    ...
    pollActiveAgents() {
        ...
        shifts.forEach(shift => {
            if(shift.breadcrumbs && shift.breadcrumbs.length > 0) {
                ...
                if(this.agentLiveMarkers[shift.userId]) { this.agentLiveMarkers[shift.userId].setLatLng([lastPing.lat, lastPing.lng]); this.agentLiveMarkers[shift.userId].getPopup().setContent(popupContent); } 
...
const AgentCtrl = {
    ...
    routeTo: function(address) { window.open(`http://googleusercontent.com/maps.google.com/6${encodeURIComponent(address)}`, '_blank'); },
    ...
    submitWork: function() { 
        let failed = false; CoreDB.getSchema().forEach(f => { if(f.tenantVisible && f.tenantMandatory) { const el = document.getElementById(`work-field-${f.id}`); if(el && !el.value) failed = true; } }); 
        if(failed) { alert("Please complete required fields."); return; } 
        alert("Work Submitted."); this.stopTimer(); UI.closeOverlay('work'); this.renderSidebarBank(); 
    },
    ...
    submitInsp: function() {
        ...
        if(flagFuture) {
            let notes = [];
            CoreDB.getSchema().forEach(f => {
                if(f.tenantVisible) { const el = document.getElementById(`insp-field-${f.id}`); if(el && el.value) notes.push(`${f.label}: ${el.value}`); }
            });
    ...
    submitReport: function() {
        ...
        alert("Issue Logged successfully to the Reporter dashboard.");
        UI.closeOverlay('report-issue');
...
const ReporterCtrl = {
    ...
    renderReports: function() {
        const list = document.getElementById('reports-list-render'); if(!list) return;
        const reports = CoreDB.getReports().filter(r => r.tenantId === CoreDB.getActiveTenantId());
        ...
        list.innerHTML = reports.map(r => {
            ...
            return `
            <div style="background:#fff; border-bottom:1px solid #eee; padding:20px; margin-bottom:10px; border-radius:8px; border-left: 4px solid ${statColor}; box-shadow:0 2px 5px rgba(0,0,0,0.05);">
                <div style="display:flex; justify-content:space-between; align-items:flex-start; margin-bottom:10px;">
                    <div>
                        <strong style="color: var(--b); font-size: 16px;">${r.type}</strong><br>
                        <span style="color: #666; font-size: 12px;">Logged: ${r.timestamp} by ${r.reportedBy}</span><br>
                        <span style="color: #666; font-size: 12px;">GPS: ${r.lat.toFixed(5)}, ${r.lng.toFixed(5)}</span>
                    </div>
                    <span class="badge" style="background: ${statColor};">${r.status}</span>
                </div>
                <div style="background:#f9f9f9; padding:10px; border-radius:5px; font-style:italic; color:#555; font-size:13px; margin-bottom:15px;">"${r.notes || 'No additional notes provided.'}"</div>
                <div style="display:flex; gap:10px;">
                    <button class="std-btn blue" style="width:auto; padding:8px 15px; font-size:12px;" onclick="ReporterCtrl.forwardReport('${r.id}')">Forward to Council ✉️</button>
                    <button class="std-btn green" style="width:auto; padding:8px 15px; font-size:12px;" onclick="ReporterCtrl.markResolved('${r.id}')">Mark Resolved ✓</button>
                </div>
            </div>`;
        }).join('');
    },
    forwardReport: function(id) {
        const r = CoreDB.getReports().find(x => x.id === id);
        const t = CoreDB.getTenants().find(x => x.id === r.tenantId);
        const email = t.reportEmail || 'info@council.govt.nz';
        const subject = encodeURIComponent(`Field Report: ${r.type}`);
        const body = encodeURIComponent(`VanGuard Field Report

Issue Type: ${r.type}
Reported By: ${r.reportedBy}
Date: ${r.timestamp}
GPS Coordinates: ${r.lat}, ${r.lng}
Map Link: http://googleusercontent.com/maps.google.com/7${r.lat},${r.lng}

Notes from Agent:
${r.notes}

(Note: Photos to be attached manually if required)`);
        window.location.href = `mailto:${email}?subject=${subject}&body=${body}`;
    },
    markResolved: function(id) {
        if(confirm("Mark this report as resolved/processed?")) {
            let reports = CoreDB.getReports();
            const idx = reports.findIndex(x => x.id === id);
            if(idx !== -1) {
                reports[idx].status = 'RESOLVED';
                CoreDB.saveReports(reports);
                this.renderReports();
            }
        }
    }
...
const DispatchCtrl = {
    // Standard Dispatch logic from previous builds...
    ...
    renderAgentTracker: function() {
        ...
        let html = '';
        if(shiftOnDate) {
            const st = new Date(shiftOnDate.startTime).toLocaleTimeString(); const et = shiftOnDate.endTime ? new Date(shiftOnDate.endTime).toLocaleTimeString() : 'Active/Open';
            html += `<div style="background:#e3f2fd; border-left:4px solid var(--b); padding:15px; border-radius:8px; margin-bottom:15px;"><h4 style="margin:0 0 5px 0;">Shift Data: ${targetDateStr}</h4><div style="font-size:13px; color:#555;">Start: <strong>${st}</strong> | End: <strong>${et}</strong></div><div style="font-size:13px; color:#555;">Status: <strong>${shiftOnDate.status}</strong></div></div>`;
...
const GodCtrl = {
    // Standard GodCtrl with new Report Email field...
