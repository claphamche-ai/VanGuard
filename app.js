// ==========================================
// VANGUARD V1.1.12 - GLOBAL PHOTO LIGHTBOX
// ==========================================
...
const UI = {
    ...
    initLightbox: function() {
        ...
        const startDrag = (clientX, clientY) => {
            if(UI.lightboxState.zoom <= 1) return;
            isDragging = true; img.style.cursor = 'grabbing';
            startX = clientX - UI.lightboxState.translateX; 
            startY = clientY - UI.lightboxState.translateY;
        };
        const doDrag = (clientX, clientY) => {
            if(!isDragging) return;
            UI.lightboxState.translateX = clientX - startX; 
            UI.lightboxState.translateY = clientY - startY;
            img.style.transform = `scale(${UI.lightboxState.zoom}) translate(${UI.lightboxState.translateX/UI.lightboxState.zoom}px, ${UI.lightboxState.translateY/UI.lightboxState.zoom}px)`;
        };
        ...
        img.ontouchstart = (e) => { startDrag(e.touches[0].clientX, e.touches[0].clientY); };
    ...
    openLightbox: function(images, startIndex) {
        this.initLightbox();
        this.lightboxState.images = images;
        this.lightboxState.currentIndex = startIndex;
        this.updateLightbox();
        document.getElementById('vg-lightbox').style.display = 'flex';
    },
    closeLightbox: function() { document.getElementById('vg-lightbox').style.display = 'none'; this.resetZoom(); },
    changeLightbox: function(dir) {
        this.lightboxState.currentIndex += dir;
        if(this.lightboxState.currentIndex < 0) this.lightboxState.currentIndex = this.lightboxState.images.length - 1;
        if(this.lightboxState.currentIndex >= this.lightboxState.images.length) this.lightboxState.currentIndex = 0;
        this.updateLightbox();
    },
    updateLightbox: function() {
        this.resetZoom();
        const imgData = this.lightboxState.images[this.lightboxState.currentIndex];
        document.getElementById('lb-img').src = imgData.url;
        document.getElementById('lb-caption').innerText = imgData.caption;
        document.getElementById('lb-counter').innerText = `${this.lightboxState.currentIndex + 1} / ${this.lightboxState.images.length}`;
        document.getElementById('lb-prev').style.display = this.lightboxState.images.length > 1 ? 'block' : 'none';
        document.getElementById('lb-next').style.display = this.lightboxState.images.length > 1 ? 'block' : 'none';
    },
    zoomLightbox: function(e) {
        e.preventDefault();
        this.lightboxState.zoom += e.deltaY * -0.005;
        this.lightboxState.zoom = Math.min(Math.max(1, this.lightboxState.zoom), 6); 
        const img = document.getElementById('lb-img');
        if(this.lightboxState.zoom === 1) { this.lightboxState.translateX = 0; this.lightboxState.translateY = 0; }
        img.style.transform = `scale(${this.lightboxState.zoom}) translate(${this.lightboxState.translateX/this.lightboxState.zoom}px, ${this.lightboxState.translateY/this.lightboxState.zoom}px)`;
    },
    resetZoom: function() {
        this.lightboxState.zoom = 1; this.lightboxState.translateX = 0; this.lightboxState.translateY = 0;
        const img = document.getElementById('lb-img');
        if(img) { img.style.transform = 'scale(1) translate(0px, 0px)'; img.style.cursor = 'grab'; }
    }
};
    ...
    showHistory: function() {
        ...
        if(jobs.length === 0) { listEl.innerHTML = `<p style="color:#666; font-size:13px;">No jobs completed on this shift yet.</p>`; } else { 
            listEl.innerHTML = jobs.map(j => {
                ...
                // Add lightbox thumbs for agent history
                if(j.photos && Object.keys(j.photos).length > 0) {
                    ...
                    for(const [step, url] of Object.entries(j.photos)) {
                        html += `<img src="${url}" style="height:60px; border-radius:3px; border:1px solid #ccc; cursor:pointer;" onclick="UI.openLightbox(${lbArrayStr}, ${pIndex})">`;
                        pIndex++;
                    }
                    html += `</div>`;
                }
...
const ReporterCtrl = {
    ...
    setJobReview: function(jobId, status) {
        ...
        if(idx !== -1) {
            bank[idx].reviewStatus = status; if(status === 'APPROVED') bank[idx].rejectReason = '';
            CoreDB.saveJobBank(bank); const localIdx = this.filteredJobsForPreview.findIndex(j => j.jobId === jobId);
            if(localIdx !== -1) this.filteredJobsForPreview[localIdx].reviewStatus = status;
            const rejBox = document.getElementById(`rej_reason_${jobId}`); if(rejBox) rejBox.style.display = status === 'REJECTED' ? 'block' : 'none';
        }
    },
    ...
    loadTemplate: function(id) {
        ...
        if(t) {
            ...
            document.getElementById('rep-date-from').value = t.fromDate; document.getElementById('rep-date-to').value = t.toDate; document.getElementById('rep-inc-pending').checked = t.incPending; document.getElementById('rep-filter-loc').value = t.filterLoc || ''; document.getElementById('rep-filter-tag').value = t.filterTag || '';
...
// ... other controllers ...
const DispatchCtrl = { /* unchanged structure */ };
const ToolsCtrl = { /* unchanged structure */ };
const AccountsCtrl = { /* unchanged structure */ };
const GodCtrl = { /* unchanged structure */ };
