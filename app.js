// ==========================================
// VANGUARD V1.1.18 - EXCEL TO KML CONVERTER
// ==========================================
...
const CoreDB = {
    ...
    getTenantJobs: function() { let jobs = this.getJobBank().filter(j => j.tenantId === this.getActiveTenantId()); const user = this.getActiveUser(); if(user && user.role === 'agent' && this.getFlags().directAssignment) { jobs = jobs.filter(j => !j.assignedTo || j.assignedTo === 'UNASSIGNED' || j.assignedTo === user.id); } return jobs; },
    getJob: function(jobId) { return this.getJobBank().find(j => j.jobId === jobId); },
    
    updateJob: function(jobId, newData, reason) {
        let b = this.getJobBank(); let idx = b.findIndex(j => j.jobId === jobId);
        if(idx !== -1) {
            let j = b[idx]; if (!j.auditLog) j.auditLog = []; let snapshot = JSON.parse(JSON.stringify(j)); delete snapshot.auditLog; 
            j.auditLog.push({ timestamp: Date.now(), dateStr: new Date().toLocaleString('en-NZ'), reason: reason, previousState: snapshot });
            j.data = newData.data; if(newData.photos) j.photos = newData.photos; if(newData.notes) j.notes = newData.notes; if(newData.srn !== undefined) j.srn = newData.srn; if(newData.assignedTo !== undefined) j.assignedTo = newData.assignedTo;
            j.reviewStatus = 'UNREVIEWED'; j.rejectReason = ''; this.saveJobBank(b);
        }
    },

    pushJob: function(jobObj) { let b = this.getJobBank(); jobObj.tenantId = this.getActiveTenantId(); jobObj.jobId = jobObj.jobId || ('J-' + Date.now() + '-' + Math.floor(Math.random() * 1000)); const activeShift = this.getActiveShift(); if(activeShift && jobObj.type !== 'PENDING') { jobObj.shiftId = activeShift.shiftId; } b.unshift(jobObj); this.saveJobBank(b); },
    ...
    createShift: function() { const user = this.getActiveUser(); if(!user) return null; let shifts = this.getShifts(); const newShift = { shiftId: 'SHIFT-' + Date.now(), tenantId: this.getActiveTenantId(), userId: user.id, username: user.name || user.username, startTime: new Date().toISOString(), endTime: null, status: 'OPEN', breadcrumbs: [], totalDistance: 0 }; shifts.push(newShift); this.saveShifts(shifts); return newShift; },
    closeShift: function() {
        let shifts = this.getShifts(); const user = this.getActiveUser(); const idx = shifts.findIndex(s => s.userId === user.id && s.status === 'OPEN');
        if(idx !== -1) { 
            let shift = shifts[idx]; shift.status = 'CLOSED'; shift.endTime = new Date().toISOString(); let dist = 0; const R = 6371;
            if(shift.breadcrumbs && shift.breadcrumbs.length > 1) {
                for(let i=1; i<shift.breadcrumbs.length; i++) {
                    ...
                    let a = Math.sin(dLat/2)*Math.sin(dLat/2) + Math.cos(p1.lat * Math.PI / 180) * Math.cos(p2.lat * Math.PI / 180) * Math.sin(dLon/2)*Math.sin(dLon/2); dist += R * (2 * Math.atan2(Math.sqrt(a), Math.sqrt(1-a)));
                }
            }
...
class MapEngine {
    constructor(mapId, role) {
        ...
        setTimeout(() => {
            ...
            this.layers = {}; this.role = role; this.userMarker = null; this.searchMarker = null; this.agentLiveMarkers = {}; this.agentLiveTrails = {}; this.loadKML(); 
        ...
        this.filteredJobsForPreview = jobs; const previewList = document.getElementById('report-preview-list'); if(jobs.length === 0) { previewList.innerHTML = `<p style="text-align:center; padding:30px; color:#888;">No jobs found matching these criteria.</p>`; } 
            ...
            jobs.forEach(j => {
                ...
                if(j.photos && Object.keys(j.photos).length > 0) {
                    ...
                    for(const [step, url] of Object.entries(j.photos)) { html += `<div style="flex: 0 0 auto; text-align:center; cursor:pointer;" onclick="UI.openLightbox(${lbArrayStr}, ${pIndex})"><img src="${url}" style="height:150px; border-radius:5px; border:1px solid #ccc; transition:transform 0.2s;" onmouseover="this.style.transform='scale(1.05)'" onmouseout="this.style.transform='scale(1)'"><div style="font-size:10px; color:#666; margin-top:5px; text-transform:uppercase; font-weight:bold;">${step}</div></div>`; pIndex++; }
                    html += `</div>`;
                }
...
// V1.1.18 EXCEL CONVERTER CONTROLLER
const ExcelConverterCtrl = {
    progress: 0, 
    init: function() { document.getElementById('excel-file-input').onchange = (e) => { const f = e.target.files[0]; document.getElementById('excel-file-name').innerText = f ? f.name : 'No file selected'; }; },
    processCSV: function() {
        const fileInput = document.getElementById('excel-file-input'); if(!fileInput.files[0]) { alert("Select a CSV file first."); return; }
        const reader = new FileReader(); reader.onload = async (e) => {
            const rows = e.target.result.split('\n').map(r => r.split(',')).filter(r => r.length > 1);
            const headers = rows[2]; // Sample file headers start on line 3
            const dataRows = rows.slice(3).filter(r => r[0]); 
            const results = [];
            const progBar = document.getElementById('excel-prog-bar');
            const statusText = document.getElementById('excel-status');
            progBar.parentElement.style.display = 'block';

            for(let i=0; i<dataRows.length; i++) {
                const r = dataRows[i];
                const addr = `${r[2]}, ${r[1]}, New Zealand`;
                statusText.innerText = `Geocoding: ${addr}`;
                
                try {
                    const res = await fetch(`https://nominatim.openstreetmap.org/search?format=json&q=${encodeURIComponent(addr)}&countrycodes=nz`);
                    const data = await res.json();
                    if(data && data.length > 0) {
                        results.push({ lat: data[0].lat, lng: data[0].lon, ref: r[0], loc: r[3], artist: r[5], owner: r[8], comments: r[10] });
                    }
                } catch(err) { console.warn("Geocode failed for " + addr); }
                
                this.progress = Math.round(((i+1)/dataRows.length) * 100);
                progBar.style.width = this.progress + '%';
                await new Promise(res => setTimeout(res, 1100)); // NOMINATIM 1SEC RATE LIMIT
            }

            this.generateKML(results);
            statusText.innerText = "Conversion Complete. KML Downloaded.";
        };
        reader.readAsText(fileInput.files[0]);
    },
    generateKML: function(data) {
        let kml = `<?xml version="1.0" encoding="UTF-8"?>\n<kml xmlns="http://www.opengis.net/kml/2.2">\n<Document>\n`;
        data.forEach(p => {
            kml += `<Placemark><name>${p.ref}: ${p.loc}</name><description><![CDATA[<b>Artist:</b> ${p.artist}<br><b>Owner:</b> ${p.owner}<br><b>Comments:</b> ${p.comments}]]></description><Point><coordinates>${p.lng},${p.lat},0</coordinates></Point></Placemark>\n`;
        });
        kml += `</Document>\n</kml>`;
        UI.downloadTextFile(`VanGuard_Inventory_${Date.now()}.kml`, kml);
    }
};
