// ==========================================
// VANGUARD V1.1.8 - BILLING ENGINE & REPORT GEN
// ==========================================
    ...
    processAndWatermarkImage: function(file, callback) {
        const reader = new FileReader(); reader.onload = function(e) {
            const img = new Image(); img.onload = function() {
                ...
                // V1.1.8 - Reduced from 1920 to 800 to save LocalStorage limit
...
const AgentCtrl = {
    activeSite: { name: "", type: "" }, workState: { startTime: null, accumulated: 0, photos: {} }, timerInterval: null, wakeLock: null, breadcrumbInterval: null,
    ...
    openOverlay: function(type) { 
        ...
        this.workState.photos = {}; // Reset photos
    ...
    handlePhoto: function(step) {
        ...
        UI.processAndWatermarkImage(input.files[0], (dataUrl) => {
            ...
            // V1.1.8 - Save photo base64 to work state for reporting
    ...
    cancelJob: function() { if(!confirm("Cancel this job?")) return; this.stopTimer(); this.workState = { startTime: null, accumulated: 0, photos: {} }; UI.closeOverlay('work'); },
    pauseJob: function() { 
...
const ReporterCtrl = {
    ...
    renderCreateForm: function() {
        ...
        if(wList) {
            ...
            agents.forEach(a => {
                wHtml += `<label style="display:flex; align-items:center; font-size:13px; margin-bottom:5px; color:#555;"><input type="checkbox" value="${a.id}" class="rep-worker-cb" checked style="margin-right:10px;"> ${a.name || a.username}</label>`;
            });
        ...
        if(mList) {
            ...
            schema.forEach(f => {
                if(f.tenantVisible) {
                    mHtml += `<label style="display:flex; align-items:center; font-size:12px; margin-bottom:5px;"><input type="checkbox" value="${f.id}" class="rep-metric-cb" checked style="margin-right:8px;"> ${f.label}</label>`;
                }
            });
        ...
        if(jobs.length === 0) {
            ...
            jobs.forEach(j => {
                ...
                html += `<div style="background:var(--nav-dark); color:white; padding:15px; display:flex; justify-content:space-between; align-items:center;">
                    <div><h3 style="margin:0;">${j.site}</h3><div style="font-size:11px; opacity:0.8;">SRN: ${j.srn || 'N/A'} | Worker: ${workerName}</div></div>
    ...
    saveTemplate: function() {
        ...
        tpls.push(template);
        CoreDB.saveReportTemplates(tpls);
        alert("Template Saved!");
        this.renderTemplates();
    },
    renderTemplates: function() {
        const list = document.getElementById('report-template-list');
        if(!list) return;
        const tpls = CoreDB.getReportTemplates().filter(t => t.tenantId === CoreDB.getActiveTenantId());
        if(tpls.length === 0) {
            list.innerHTML = '<p style="color:#888; text-align:center; padding:20px;">No saved templates.</p>';
            return;
        }
    ...
    loadTemplate: function(id) {
        const tpls = CoreDB.getReportTemplates();
        ...
        if(t) {
            ...
            const allW = document.querySelectorAll('.rep-worker-cb');
            const checkedW = document.querySelectorAll('.rep-worker-cb:checked');
    ...
    exportWord: function() {
        ...
        jobs.forEach(j => {
            ...
            html += `<div class="job-block">
                <div class="header">Site: ${j.site} | SRN: ${j.srn || 'N/A'}</div>
                <table>
                    <tr><td><strong>Agent:</strong></td><td>${wName}</td><td><strong>Date:</strong></td><td>${j.pausedAt}</td></tr>
            `;

            if(j.data) {
                for(let i=0; i<selectedMetrics.length; i+=2) {
                    const m1 = selectedMetrics[i]; const f1 = schema.find(x => x.id === m1);
                    const v1 = j.data[m1] || '';
                    
                    let m2 = null, f2 = null, v2 = '';
                    if(i+1 < selectedMetrics.length) {
                        m2 = selectedMetrics[i+1]; f2 = schema.find(x => x.id === m2);
                        v2 = j.data[m2] || '';
                    }

                    html += `<tr>
                        <td><strong>${f1?f1.label:m1}:</strong></td><td>${v1}</td>
                        ${m2 ? `<td><strong>${f2?f2.label:m2}:</strong></td><td>${v2}</td>` : `<td></td><td></td>`}
                    </tr>`;
                }
            }
            ...
            if(j.photos && Object.keys(j.photos).length > 0) {
                ...
                for(const [step, url] of Object.entries(j.photos)) {
                    html += `<span><strong>${step.toUpperCase()}</strong><br><img src="${url}"></span>`;
                }
                html += `</div>`;
            }
            html += `</div>`;
